    ...
    <div className="max-w-5xl mx-auto animate-fade-in">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm">{clientes.length} cliente(s) cadastrado(s)</p>
        </div>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          style={{ paddingTop: '14px', paddingBottom: '14px', paddingLeft: '44px' }}
        />
      </div>

      {filtrados.length === 0 ? (
        <div className="card p-16 text-center">
          <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-5" style={{ background: '#e8edf4' }}>
            <Users size={36} style={{ color: '#1a3050' }} />
          </div>
          {busca ? (
            <>
              <h3 className="text-xl font-bold mb-2" style={{ color: '#1a3050' }}>Nenhum resultado encontrado</h3>
              <p className="text-gray-400 mb-6">Tente buscar por outro nome ou CPF</p>
              <button onClick={() => setBusca('')} className="btn-outline">Limpar busca</button>
            </>
          ) : (
            <>
              <h3 className="text-xl font-bold mb-2" style={{ color: '#1a3050' }}>Nenhum cliente cadastrado</h3>
              <p className="text-gray-400 mb-8 max-w-sm mx-auto">Cadastre seu primeiro cliente e comece a gerenciar seus contratos</p>
              <Link href="/clientes/novo" className="btn-primary">
                <Plus size={16} />
        ...
        <div className="card overflow-hidden">
          <div className="divide-y divide-gray-50">
            {filtrados.map((cliente, idx) => {
              ...
              return (
                <div key={cliente.id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors group">
                  <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold text-white text-sm flex-shrink-0" style={{ background: cor }}>
                    {inicial}
                  </div>
                  ...
                  <Link href={`/clientes/${cliente.id}`} className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5 flex items-center gap-3">
                      {cliente.dados.CPF_CLIENTE && <span>CPF: {cliente.dados.CPF_CLIENTE}</span>}
                      {cliente.dados.CIDADE_CLIENTE && <><span>·</span><span>{cliente.dados.CIDADE_CLIENTE}</span></>}
                      <span>·</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link href={`/clientes/${cliente.id}`} className="opacity-0 group-hover:opacity-100 transition-opacity btn-ghost py-1.5 px-3 text-xs">
